...
namespace SQVector
{
	SQInteger Construct(HSQUIRRELVM vm)
	{
		...
		*/
		...
		SQUserPointer p;
		if (SQ_FAILED(sq_getinstanceup(vm, 1, &p, 0, SQTrue)))
		{
			return SQ_ERROR;
		}
...
bool getVariant(HSQUIRRELVM vm, SQInteger idx, ScriptVariant_t& variant)
{
	...
	{
		Vector* v = nullptr;
		if (SQ_SUCCEEDED(sq_getinstanceup(vm, idx, (SQUserPointer*)&v, TYPETAG_VECTOR, SQTrue)))
		{
			variant.Free();
...
SQInteger function_stub(HSQUIRRELVM vm)
{
	...
	if (pFunc->m_flags & SF_MEMBER_FUNC)
	{
		...
		if (SQ_FAILED(sq_getinstanceup(vm, 1, (SQUserPointer*)&classInstanceData, 0, SQTrue)))
		{
			return SQ_ERROR;
		}
...
HSCRIPT SquirrelVM::RegisterInstance(ScriptClassDesc_t* pDesc, void* pInstance, bool bRefCounted)
{
...

	{
		...
		sq_getinstanceup(vm_, -1, (SQUserPointer*)&self, 0, SQTrue);
...
void SquirrelVM::SetInstanceUniqeId(HSCRIPT hInstance, const char* pszId)
{
	...
	ClassInstanceData* classInstanceData;
	sq_getinstanceup(vm_, -1, (SQUserPointer*)&classInstanceData, nullptr, SQTrue);
...
void SquirrelVM::RemoveInstance(HSCRIPT hInstance)
{
	...
	sq_pushobject(vm_, *obj);
	sq_getinstanceup(vm_, -1, (SQUserPointer*)&self, nullptr, SQTrue);
...
// @NMRiH - Felis: Getting a bunch of 64->32 integer precision warnings here when compiling under x64
// Would have to add a new CUtlBuffer function that accepts Put() with size_t or equivalent
// Squelch those for now since it appears to be negligible here...
#ifdef _WIN32
#pragma warning( push )
...
// @NMRiH - Felis
// REVISIT: This only exists for legacy support
HSCRIPT SquirrelVM::DuplicateObject(const HSCRIPT hObject)
{
	return CopyObject(hObject);
}
